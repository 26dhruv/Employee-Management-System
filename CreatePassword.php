...
<body>
    ...
    <div class="container">
        ...
        <form action="./CreatePassword.php" method="POST">
            <div class="email">
                <input type="email" name="user_email" id="EMAIL" readonly required pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$" title="Enter email in proper formate">
                <span>Email</span>
                <i></i>
            </div>
            <div class="password">
        ...
        if (isset($_POST['submit'])) {
            ...
            if ($c_pass == $pass) {
                ...
                if (!$result) {
                    ...
                    $update_pass = "UPDATE `user_master` SET `u_pass` = '$pass', `is_active` = '0' WHERE `user_master`.`u_id` = $id;";
                    $result1 = mysqli_query($conn, $update_pass);
                    if (!$result1) {
                        echo "<script>alert(\Upadation Error\");</script>";
                    } else {
                        echo "<script>alert(\"Password Successfully Set\");</script>";
                    }
                }
            } else {
