...
<body>
    ...
    $_SESSION['leave_assign_deleted'] = 2;
    if (!$_SESSION['Logged_in'] || $_SESSION['u_role_id'] != 0) {

        header("Location: ../LoginPage.php");
        exit;
    }
    if ($_SESSION['leave_assign_deleted'] == 1) {
        echo "<script>alert('leave Deleted')</script>";
        unset($_SESSION['leave_assign_deleted']);
    } elseif ($_SESSION['leave_assign_deleted'] == 0) {
        echo "<script>alert('leave Deletion Error')</script>";
        unset($_SESSION['leave_assign_deleted']);
    }
