...
<body>
    ...
    <form action="./DocType.php" id="Register_Form" method="post">
        <div class="DocType">
            <input type="text" name="DocType" id="DocType" required maxlength="32" pattern="[A-Za-z]{3,32}" title="Must contain only character">
    ...
    if (isset($_POST['DocType'])) {
        ...
        $get_dep = "SELECT * FROM `doc_type_master` WHERE `doc_type`='$doc_type' ";
        $resultdep = mysqli_query($conn, $get_dep);

        if (mysqli_num_rows($resultdep) > 0) {
            echo "<script>alert('Document type  already exists')</script>";
    ?><a href="./DocType.php"></a><?php
                                } else {


                                    $query1 = "INSERT INTO `doc_type_master` (`doc_type`) VALUES ('$doc_type')";
                                    mysqli_query($conn, $query1);
                                    echo "<script>alert('Document Type Added');</script>";
                                    ?><a href="./DocType.php"></a><?php
                                                                }
                                                                //  else
    ...
    <div class="Main_Content">
        <table>
            ...
            <tbody id="table-body">
                ...
                <?php
                include "Config.php";
                $query = "SELECT * FROM `doc_type_master`;";
                ...
                ?>
                    <tr>
                        ...
                        <form method="post" action="./DelDocType.php">
    ...
    <script>
        ...
        searchInput.addEventListener("input", function() {
            ...
            xhr.open("GET", "search.php?Doc_type=" + encodeURIComponent(query), true);
