...
<head>
    ...
    <link rel="stylesheet" href="./AddDegree.css">
    <!-- font link -->
...
<body>
    ...
    <div class="main">
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            $degree_name = $_POST['U_Degree'];



            $get_dep = "SELECT * FROM `degree_master` WHERE `degree_field`='$degree_name' ";
            $resultdep = mysqli_query($conn, $get_dep);

            if (mysqli_num_rows($resultdep) > 0) {
                echo "<script>alert('Degree already exists')</script>";
        ?><a href="./AddDegree.php"></a><?php
                                    } else {

                                        $query = "INSERT INTO `degree_master` (`degree_field`) VALUES ('$degree_name');";
                                        mysqli_query($conn, $query);
                                        echo "<script>alert('Degree added successfully')</script>";
                                        ?><a href="./AddDegree.php"></a><?php
                                                                    }
                                                                }
                                                                        ?>
    ...
    <form action="./AddDegree.php" method="POST" id="Form">

        <div class="Degree">
            <input type="text" name="U_Degree" id="U_Degree" required maxlength="32" pattern="[a-zA-Z./]{2,32}" title="Must contain only Lowercase character and Period">
            <span>Degree</span>
            <i></i>
        </div>
    ...
    <div class="Main_Content">
        <table>
            ...
            <tbody id="table-body">
                ...
                <?php
                include "Config.php";
                $query = "SELECT * FROM `degree_master` ORDER BY `degree_master`.`degree_id` ASC;";
    ...
    <script>
        ...
        searchInput.addEventListener("input", function() {
            ...
            xhr.open("GET", "search.php?deg_id=" + encodeURIComponent(query), true);
