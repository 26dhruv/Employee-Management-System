...
<head>
    ...
    <!-- css link -->
    <link rel="stylesheet" href="/Project/Admin/Template.css">
    <link rel="stylesheet" href="/Project/Admin/AddState.css">
    <!-- font link -->
...
<body>
    ...
    <form action="./AddState.php" method="POST" id="Form">
        <div class="State">
            <input type="text" name="U_State" id="STATE" required maxlength="32" pattern="[a-z\s]{2,32}" title="Must contain only character">
            <span>State</span>
            <i></i>
        </div>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $get_dep = "SELECT * FROM `state_master` WHERE `state`='$state' ";
        $resultdep = mysqli_query($conn, $get_dep);

        if (mysqli_num_rows($resultdep) > 0) {
            echo "<script>alert('State already exists')</script>";
    ?><a href="./AddState.php"></a><?php
                                } else {
                                    $query = "INSERT INTO `state_master` ( `state`) VALUES ( '$state');";
    ...
    <script>
        ...
        searchInput.addEventListener("input", function() {
            ...
            xhr.open("GET", "search.php?state_id=" + encodeURIComponent(query), true);
